    ...
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold text-white">Project exposure dashboard</h1>
        ...
        <div className={`rounded-full border px-3 py-1.5 text-xs font-semibold ${running ? "border-sky-500/40 bg-sky-500/10 text-sky-300" : "border-slate-700 bg-slate-900 text-slate-400"}`}>
      ...
      <div className="grid grid-cols-[1.5fr_1fr] gap-5">
        <div className="panel overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-800 px-5 py-4">
            <div>
              <div className="font-semibold text-slate-100">Highest-signal projects</div>
              <div className="mt-1 text-xs text-slate-500">Sorted by number of matched patterns</div>
            </div>
          </div>
              ...
              recent.map((finding) => (
                <div key={finding.repository.full_name} className="flex items-center justify-between px-5 py-4">
                  <div className="min-w-0">
                    <div className="truncate font-medium text-slate-200">{finding.repository.full_name}</div>
                    <div className="mt-1 text-xs text-slate-500">
                      {finding.repository.language ?? "Unknown"} · {finding.scanned_files} files · {finding.repository.stars} stars
                    </div>
                  </div>
                  <div className="ml-4 rounded-lg border border-slate-700 bg-slate-950 px-3 py-1.5 text-sm font-semibold text-sky-300">
        ...
        <div className="panel p-5">
          <div className="font-semibold text-slate-100">Safety boundaries</div>
